...
void main() {
    // Create gradient between Acira Blue and Cyan
    float distort = 2.0 * vDisplacement * u_intensity * sin(vUv.y * 10.0 + u_time);
    
    // Mix between blue and cyan based on displacement and UV
    vec3 color = mix(u_colorPrimary, u_colorSecondary, vUv.y + distort * 0.5);
    
    // Add subtle brightness variation
    color = mix(color, vec3(1.0), abs(distort) * 0.08);
...
const Blob = ({ isDarkMode }) => {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      u_colorPrimary: { value: new Color(isDarkMode ? '#0d52ff' : '#0d52ff') }, // Acira Blue
      u_colorSecondary: { value: new Color(isDarkMode ? '#00d4ff' : '#00d4ff') }, // Acira Cyan
  ...
  useFrame((state) => {
    ...
    if (mesh.current) {
      ...
      material.uniforms.u_noiseScale.value =
        Math.sin(clock.getElapsedTime() * 0.1) * 1 + 1;
  ...
  return (
    <div style={{
      ...
      >
        ...
        <EffectComposer>
          <Noise premultiply blendFunction={BlendFunction.ADD} opacity={0.1} />
        </EffectComposer>
      </Canvas>
    </div>
  );
